#!/bin/bash

# Скрипт для обновления проекта на сервере
# Использование: ./update-my-shop.sh

set -e  # Прерывать выполнение при ошибке

# Логирование
LOG_FILE="${HOME}/my-shop-deploy.log"
exec > >(tee -a "$LOG_FILE") 2>&1

echo "=========================================="
echo "🚀 Начало деплоя: $(date '+%Y-%m-%d %H:%M:%S')"
echo "=========================================="

# Проверка окружения
echo '🔍 Проверка окружения...'

# Проверка Node.js
if ! command -v node &> /dev/null; then
  echo '❌ Node.js не установлен'
  exit 1
fi
NODE_VERSION=$(node -v)
echo "✅ Node.js: $NODE_VERSION"

# Проверка npm
if ! command -v npm &> /dev/null; then
  echo '❌ npm не установлен'
  exit 1
fi
NPM_VERSION=$(npm -v)
echo "✅ npm: $NPM_VERSION"

# Проверка PM2
if ! command -v pm2 &> /dev/null; then
  echo '❌ PM2 не установлен. Установите: npm install -g pm2'
  exit 1
fi
PM2_VERSION=$(pm2 -v)
echo "✅ PM2: $PM2_VERSION"

# Проверка Git
if ! command -v git &> /dev/null; then
  echo '❌ Git не установлен'
  exit 1
fi
echo "✅ Git установлен"

# Определяем путь к проекту
# Если скрипт запущен из папки scripts, переходим на уровень выше
# Иначе используем домашнюю директорию
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"

# Проверяем, что мы в правильной директории (должен быть package.json)
if [ ! -f "$PROJECT_DIR/package.json" ]; then
  # Если не нашли, пробуем домашнюю директорию
  PROJECT_DIR="$HOME/my-shop"
  if [ ! -f "$PROJECT_DIR/package.json" ]; then
    echo '❌ Папка проекта my-shop не найдена'
    echo "Искали в: $PROJECT_DIR"
    exit 1
  fi
fi

echo '📁 Переход в папку проекта...'
echo "Путь: $PROJECT_DIR"
cd "$PROJECT_DIR" || { echo '❌ Не удалось перейти в папку проекта'; exit 1; }

# Создание резервной копии текущей версии
BACKUP_DIR="$PROJECT_DIR/.backup-$(date +%Y%m%d-%H%M%S)"
if [ -d ".next" ]; then
  echo '💾 Создание резервной копии...'
  mkdir -p "$BACKUP_DIR"
  cp -r .next "$BACKUP_DIR/" 2>/dev/null || true
  echo "✅ Резервная копия создана: $BACKUP_DIR"
fi

echo '🔄 Обновление из Git...'
git fetch origin main

echo '🌿 Переключение на ветку main...'
git checkout main || git checkout -b main origin/main

echo '🗑 Очистка локальных изменений...'
git reset --hard origin/main
git clean -fd

echo '✅ Репозиторий синхронизирован с origin/main'

echo '🛑 Остановка PM2 перед установкой зависимостей...'
pm2 stop my-shop 2>/dev/null || true
pm2 delete my-shop 2>/dev/null || true
sleep 2

echo '🗑 Полная очистка node_modules и package-lock.json...'
rm -rf node_modules
rm -f package-lock.json
sleep 1

echo '🧹 Очистка npm кэша...'
npm cache clean --force 2>/dev/null || true
sleep 1

echo '📦 Установка зависимостей...'
echo "Начало установки: $(date '+%Y-%m-%d %H:%M:%S')"
# Увеличиваем таймаут до 20 минут (1200 секунд) для больших проектов
if timeout 1200 npm ci --prefer-offline --no-audit --legacy-peer-deps 2>&1; then
  echo "✅ Зависимости установлены успешно"
else
  EXIT_CODE=$?
  echo "⚠️ npm ci завершился с кодом $EXIT_CODE, пробуем npm install..."
  if timeout 1200 npm install --legacy-peer-deps 2>&1; then
    echo "✅ Зависимости установлены через npm install"
  else
    echo '❌ Ошибка при установке зависимостей'
    echo 'Проверьте логи выше для деталей'
    exit 1
  fi
fi
echo "Завершение установки: $(date '+%Y-%m-%d %H:%M:%S')"

echo '🛠 Сборка проекта...'
echo "Начало сборки: $(date '+%Y-%m-%d %H:%M:%S')"
# Увеличиваем таймаут до 30 минут для сборки
if timeout 1800 npm run build 2>&1; then
  echo "✅ Сборка завершена успешно"
else
  echo '❌ Ошибка при сборке проекта'
  echo 'Проверьте логи выше для деталей'
  exit 1
fi
echo "Завершение сборки: $(date '+%Y-%m-%d %H:%M:%S')"

# Проверка наличия переменных окружения
if [ ! -f ".env.local" ] && [ ! -f ".env.production" ] && [ ! -f ".env" ]; then
  echo "⚠️  Предупреждение: файл .env не найден. Убедитесь, что переменные окружения настроены."
fi

echo '🚀 Перезапуск сервиса через PM2...'
# Создаём директорию для логов если её нет
mkdir -p "$PROJECT_DIR/logs"

# Проверяем наличие конфигурации PM2
if [ -f "$PROJECT_DIR/ecosystem.config.cjs" ]; then
  echo "📋 Использование ecosystem.config.cjs для PM2"
  pm2 start ecosystem.config.cjs || { echo '❌ Ошибка при запуске PM2'; exit 1; }
  echo "✅ PM2 сервис запущен через ecosystem.config.cjs"
elif [ -f "$PROJECT_DIR/ecosystem.config.js" ]; then
  echo "📋 Использование ecosystem.config.js для PM2"
  pm2 start ecosystem.config.js || { echo '❌ Ошибка при запуске PM2'; exit 1; }
  echo "✅ PM2 сервис запущен через ecosystem.config.js"
else
  echo "⚠️ ecosystem.config не найден, используем базовую конфигурацию"
  pm2 start npm --name my-shop -- start || { echo '❌ Ошибка при запуске PM2'; exit 1; }
  echo "✅ PM2 сервис запущен"
fi

# Проверка статуса приложения
echo '🔍 Проверка статуса приложения...'
sleep 5
PM2_STATUS=$(pm2 jlist | grep -o '"status":"[^"]*"' | head -1 || echo "")
if echo "$PM2_STATUS" | grep -q "online"; then
  echo "✅ Приложение запущено и работает"
else
  echo "❌ Приложение не запущено или имеет проблемы"
  echo "Статус PM2:"
  pm2 status
  echo "Логи PM2:"
  pm2 logs my-shop --lines 20 --nostream
  exit 1
fi

# Очистка старых резервных копий (оставляем последние 3)
echo '🧹 Очистка старых резервных копий...'
cd "$PROJECT_DIR"
ls -dt .backup-* 2>/dev/null | tail -n +4 | xargs rm -rf 2>/dev/null || true

echo "=========================================="
echo '✅ Деплой успешно завершён.'
echo "Время завершения: $(date '+%Y-%m-%d %H:%M:%S')"
echo "Лог сохранён в: $LOG_FILE"
echo "=========================================="
