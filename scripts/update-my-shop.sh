...
if timeout 600 npm ci --prefer-offline --no-audit; then
  ...
  echo "⚠️ npm ci завершился с ошибкой, пробуем npm install..."
  timeout 600 npm install || { echo '❌ Ошибка при установке зависимостей'; exit 1; }
fi
echo "Завершение установки: $(date '+%Y-%m-%d %H:%M:%S')"

echo '🛠 Сборка проекта...'
echo "Начало сборки: $(date '+%Y-%m-%d %H:%M:%S')"
timeout 900 npm run build || { echo '❌ Ошибка при сборке проекта'; exit 1; }
