...
import PhysicalDisk from "../model/PhysicalDisk.js";
    ...
    data.platforms.map(async (platform) => {
      try {
        return await PhysicalDisk.create({
          gameId: game._id,
          platform,
          variants: [{ condition: "new", stock: 0, price: 0 }],
        });
      } catch (error) {
        console.error(`Ошибка при создании физического диска для ${platform}:`, error);
        return null;
      }
    })
  );

  return { game, physicalDisks: physicalDisks.filter(Boolean) };
}
