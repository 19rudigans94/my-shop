...
export default function CartItems() {
  ...
  const getItemKey = (item) => {
    return `${item.id}_${item.condition || "new"}_${item.variant || "physical"}`;
  };
            ...
            {cartItems.map((item) => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() =>
                      handleQuantityChange(item.id, item.quantity - 1, item.condition, item.variant)
                    }
                ...
                <button
                  onClick={() => handleRemoveItem(item.id, item.condition, item.variant)}
